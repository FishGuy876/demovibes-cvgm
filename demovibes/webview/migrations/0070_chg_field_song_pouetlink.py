...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print "Nulling pouet_link with length > 98 ..."
        total = orm.Song.objects.all().count()
        numberSong = 1
        for song in orm.Song.objects.all():
            Q = orm.Queue.objects.filter(song=song).order_by('-id')
            if len(song.pouetlink) > 98:
				song.pouetlink = ""
				song.save()
				print "%d/%d CHANGED" % (numberSong, total)
            else:
				print "%d/%d PASSED" % (numberSong, total)
            numberSong = numberSong+1
        print "Emptying pouet_link with length > 99 ... DONE"
    ...
    models = {
        ...
        'webview.userprofile': {
            ...
            'last_changed': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2011, 2, 17, 2, 22, 0, 899087)'}),
