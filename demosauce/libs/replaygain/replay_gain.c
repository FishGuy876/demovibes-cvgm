...
{
    // there is a strange bug in the replaygain code that can cause GetTitleGain a
    // inplausible value if frames is odd. this should be a temp fix, but you know....
    frames &= ~1;
    float** buffer = data;
...
void rg_analyze(struct rg_context* ctx, void* data, int frames)
{
    assert(ctx);
    assert(data);
    // assert(frames % 2 == 0); // odd number of frames sometimes causes odd behaviour
...
double rg_title_gain(struct rg_context* ctx)
{
    double gain = GetTitleGain(&ctx->state);
    return gain == GAIN_NOT_ENOUGH_SAMPLES ? 0 : gain;
}

double rg_album_gain(struct rg_context* ctx)
{
    double gain = GetAlbumGain(&ctx->state);
    return gain == GAIN_NOT_ENOUGH_SAMPLES ? 0 : gain;
}
