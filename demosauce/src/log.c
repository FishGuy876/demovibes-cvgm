...
static void fvlog(FILE* f, enum log_level lvl, const char* fmt, va_list args)
{
    ...
    char buf[30] = {};
    time(&rawtime);
    if (!strftime(buf, sizeof buf, "%Y-%m-%d %X", localtime(&rawtime)))
        buf[0] = 0;
    fprintf(f, "%s %s ", levels[lvl - 1], buf);
    vfprintf(f, fmt, args);
    fputc('\n', f);
    fflush(f);
}
...
{
    const char* lstr[] = {"debug", "info", "warn", "error", "fatal", "off", NULL};
    enum log_level llvl[] = {LOG_DEBUG, LOG_INFO, LOG_WARN, LOG_ERROR, LOG_FATAL, LOG_OFF, 0};
    for (int i = 0; lstr[i] && llvl[i]; i++) {
