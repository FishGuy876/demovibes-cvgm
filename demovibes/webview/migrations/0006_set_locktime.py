...
class Migration:
    ...
    def forwards(self, orm):
        ...
        print " Populating locked_until field.."
        total = orm.Song.objects.all().count()
        sl = settings.SONG_LOCK_TIME
        lock_time = datetime.timedelta(hours = sl['hours'], days = sl['days'], minutes = sl['minutes'])
        for song in orm.Song.objects.all().order_by('id'):
            Q = orm.Queue.objects.filter(song=song).order_by('-id')
            if Q:
                last_queued = Q[0].requested
                song.locked_until = last_queued + lock_time
                print "  %d/%d Song '%s', last played %s, lock time %s, song locked to %s" % (song.id, total, song.title, last_queued, lock_time, song.locked_until)
                song.save()
            else:
                print "  %d/%d Song '%s', never played. Skipping" % (song.id, total, song.title)
        print " Populating locked_until field done, remember to restart stream player"
